...
def run_vae(experiment_name, structure_shape, pop_size, max_evaluations, train_iters, num_cores, args, generations):
    ...
    # =======================================================================================
        ...
        # int(structure.label[:-3])%2+2，用2、3两张卡
            ppo_args = (int(structure.label[:-3])%2+2, args, task_id_name_mapper[structure.task_id], structure, tc, (save_path_controller, structure.label))
...
if __name__ == "__main__":
    ...
    # env_tasks = ["Walker-v0", "Carrier-v0"]
    env_tasks = ["Flipper-v0","Jumper-v0","Balancer-v0"]#["Walker-v0", "Climber-v0", "DownStepper-v0"]
    task_name_id_mapper = {}
    task_id_name_mapper = {}
    ...
    experiment_name = "morphVAE_att4_try7(s01,no MP)" + time.strftime("%Y-%m-%d-%H_%M_%S", time.localtime(time.time()))
