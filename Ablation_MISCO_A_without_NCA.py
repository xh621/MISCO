...
from CVAE_wo_NCA_coparam import VAE
...
def run_vae(experiment_name, structure_shape, pop_size, max_evaluations, train_iters, num_cores, args, generations):
    ...
    model = VAE(y_size = args.num_tasks, x_size = args.width,
            ...
            y_emb = 128, h_emb = 128, hij_emb = 64,
            heads = args.heads, att_dim = 64, 
            config_enum="parallel") # VAE形态模型的初始化
    
    '''
    dic = torch.load('params/mymodelparams.pt')
    params = {}
    for key in dic["params"].keys():
        params[key[9:]] = dic["params"][key]
    model.load_state_dict(params)
    print("Successfully load in VAE params!")
    '''
...
if __name__ == "__main__":
    ...
    seed = 0 # 1,2#注意torch的seed在get_arg()里面，也就是ppo文件夹的argument_sjr里面还有个seed,下面接口调出来直接改了
    random.seed(seed)
    np.random.seed(seed)
    ...
    # env_tasks = ["Walker-v0", "Climber-v0", "DownStepper-v0", "Catcher-v0", "Carrier-v0", "Pusher-v0"]
    env_tasks = ["Catcher-v0", "Carrier-v0", "Pusher-v0"]#["PlatformJumper-v0","GapJumper-v0","Traverser-v0"]#["Walker-v0", "Climber-v0", "DownStepper-v0"]#["Catcher-v0", "Carrier-v0", "Pusher-v0"]
    #env_tasks =["ObstacleTraverser-v0","GapJumper-v0","Traverser-v0"]#["Flipper-v0","Jumper-v0","Balancer-v0"]#["PlatformJumper-v0","ObstacleTraverser-v0","Traverser-v0"]# ["Flipper-v0","Jumper-v0","Balancer-v0"]#["PlatformJumper-v0","GapJumper-v0","Traverser-v0"]# ["PlatformJumper-v0","ObstacleTraverser-v0","Traverser-v0"]#["Hurdler-v0","GapJumper-v0","Traverser-v0"]#["Flipper-v0","Jumper-v0","Balancer-v0"]#["AreaMaximizer-v0","WingspanMaximizer-v0","HeightMaximizer-v0"]# ["CaveCrawler-v0","BeamSlider-v0","BeamToppler-v0"]#["PlatformJumper-v0","GapJumper-v0","Traverser-v0"]
    #env_tasks =["ObstacleTraverser-v0", "Hurdler-v0","PlatformJumper-v0"]
    #try1:["BidirectionalWalker-v0", "Lifter-v0", "Hurdler-v0"]
    #env_tasks = ["Climber-v0"]
    task_name_id_mapper = {}
    task_id_name_mapper = {}
    ...
    #CVAE_att4_add3(adv,D,sim0,seed:2,1: no MP)
    experiment_name = "Ab_add3_CVAE_att4_wo_nca_coparam(adv, sim:0, seed:0,1: No MP)" + time.strftime("%Y-%m-%d-%H_%M_%S", time.localtime(time.time()))
